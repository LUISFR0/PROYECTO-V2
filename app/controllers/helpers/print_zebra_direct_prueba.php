...
// ===========================
// GUARDAR EN COLA (un job por etiqueta)
// ===========================
try {
...
^XZ
";
        $stmt->execute([$zpl_item]);
    }

    ob_clean();
    echo json_encode(['status' => 'success', 'message' => count($stocks) . ' etiquetas enviadas a la cola de impresión']);
    exit;
} catch (PDOException $e) {
    ob_clean();
